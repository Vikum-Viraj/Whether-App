...
const TemperatureGraph = ({ weatherData }) => {
    ...
    const chartData = weatherData?.map(city => ({
        ...
        temperatureF: parseFloat(kelvinToFahrenheit(city.weather.temp)),
    ...
    return (
        <div className="max-w-5xl mx-auto">
            <div className="bg-white rounded-lg shadow-sm border border-blue-200 p-5 mt-6">
                <h2 className="text-2xl font-bold text-blue-900 mb-4 text-center">Temperature Trends per City</h2>
                <ResponsiveContainer width="100%" height={400}>
                    <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="2 2" />
                        <XAxis dataKey="name" label={{ value: 'Cities', position: 'insideBottom', offset: -5 }} />
